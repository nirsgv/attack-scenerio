...
function Steps({ steps, team_id, updateCampaign }) {
    ...
    <div className={`steps ${entranceClassName}`} onAnimationEnd={() => setEntranceClassName('')}>
        <div className="title__wrap">
            <h2 className={'title__steps'}>Current</h2>
        </div>

        {inProgressGroup[0] && <InProgressStep step={inProgressGroup[0]} updateCampaign={updateCampaign} team_id={team_id}/>}
        <div classname="title__wrap">
            <h2 className={'title__steps'}>Next</h2>
        </div>
        <List baseClassName={'to-handle'}>
            {toHandleGroup.map((item, i) => <ToHandleStep step={item} key={i} team_id={team_id}/>)}
        </List>
        <div classname="title__wrap">
            <h2 className={'title__steps'}>Done</h2>
        </div>
